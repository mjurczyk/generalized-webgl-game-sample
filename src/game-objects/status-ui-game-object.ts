...
export class StatusUiGameObject {
  static onCreate() {
    ...
    preloadTextureList({
      uiElements: ['assets/game-objects/status-ui-game-object/ui-frame.png'],
    });
    ...
    const heartsCounterStructure: GameObjectGeneralRenderingBlueprint[] = Array(5)
      .fill(0)
      .map(
        (_, index) =>
    ...
    ScheduleService.registerFrameListener(() => {
      const { props } =
        promptStructure.icon as GameObjectRenderingBlueprint<GameObjectTypeEnum.Sprite>;
