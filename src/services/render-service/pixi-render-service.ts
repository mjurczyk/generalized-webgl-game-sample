...
class PixiRenderServiceClass implements IAbstractRenderService<PIXI.DisplayObject> {
  ...
  getFrustumScaling(): [number, number] {
    ...
    return [
      (this.renderer.view.width / this.renderer.view.height * frustumBaseFactor) * window.devicePixelRatio,
        ...
        if (!binding || node.needsUpdate) {
          ...
          if (renderPerser) {
            ...
            if (node.children?.length) {
              node.children.forEach(child => this.sceneBindings[child.id!] && renderedObject.addChild(this.sceneBindings[child.id!]));
            }
